...
const BRANCHES = [
  { d: 'M240 168 C239 139 240 109 239 72', gate: 0.04 },
  { d: 'M239 126 C218 113 201 99 187 79', gate: 0.2 },
  { d: 'M239 116 C263 103 280 89 291 68', gate: 0.28 },
  { d: 'M224 106 C213 91 207 76 207 61', gate: 0.4 },
  { d: 'M255 104 C268 91 274 79 278 57', gate: 0.48 },
  { d: 'M239 88 C228 76 224 64 226 48', gate: 0.58 },
  { d: 'M240 86 C250 73 255 61 253 43', gate: 0.66 },
] as const;

const LEAVES = [
  { x: 186, y: 77, r: 16, gate: 0.27 },
  { x: 205, y: 61, r: 18, gate: 0.42 },
  { x: 225, y: 47, r: 19, gate: 0.58 },
  { x: 253, y: 42, r: 18, gate: 0.66 },
  { x: 278, y: 56, r: 20, gate: 0.5 },
  { x: 292, y: 68, r: 16, gate: 0.35 },
  { x: 233, y: 72, r: 22, gate: 0.72 },
  { x: 260, y: 70, r: 21, gate: 0.76 },
  { x: 211, y: 82, r: 18, gate: 0.64 },
  { x: 276, y: 86, r: 17, gate: 0.72 },
] as const;
  ...
  return (
    ...
    >
      ...
      <svg className="scene-tree__art" viewBox="0 0 480 185" preserveAspectRatio="none" aria-hidden="true">
        ...
        <g className="scene-tree__sway">
          {BRANCHES.map(branch => {
            const branchGrowth = Math.max(0, Math.min(1, (growth - branch.gate) / Math.max(0.12, 1 - branch.gate)));
            return (
              <g key={branch.d}>
                <path
          ...
          <g className="scene-tree__canopy">
            {LEAVES.map((leaf, index) => {
              const reveal = Math.max(0, Math.min(1, (growth - leaf.gate) / 0.18));
              return (
                <g
                  key={`${leaf.x}-${leaf.y}`}
                  ...
                  style={{
                    opacity: reveal,
                    transform: `translate(${leaf.x}px, ${leaf.y}px) scale(${0.35 + reveal * 0.65})`,
