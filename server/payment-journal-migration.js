...
export function legacyPaymentReview(item, providerPayments) {
  ...
  if (matches.some(payment => !payment.id || payment.status !== 'canceled'
    || payment.metadata?.subscription_id !== item.id
    || (payment.merchant_customer_id != null && payment.merchant_customer_id !== item.id)
