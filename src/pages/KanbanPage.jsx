...
const LS_KEY_TIMESTAMPS = 'memora-feedback-timestamps';

function getStoredTimestamps() {
    try {
        const raw = localStorage.getItem(LS_KEY_TIMESTAMPS);
        if (!raw) return [];
        return JSON.parse(raw).filter(ts => Date.now() - ts < RATE_LIMIT_COOLDOWN_MS);
    } catch {
        return [];
    }
}

function storeTimestamps(timestamps) {
    localStorage.setItem(LS_KEY_TIMESTAMPS, JSON.stringify(timestamps));
}
...
export default function KanbanPage() {
    ...
    const isOnCooldown = timestamps.length >= RATE_LIMIT_MAX;
    const messagesLeft = RATE_LIMIT_MAX - timestamps.length;
    ...
    useEffect(() => {
        ...
        const endTime = oldestTs + RATE_LIMIT_COOLDOWN_MS;
    ...
    const handleFeedback = useCallback(() => {
        if (!feedbackText.trim() || isOnCooldown) return;

        const now = Date.now();
        const newTimestamps = [...timestamps.filter(ts => now - ts < RATE_LIMIT_COOLDOWN_MS), now];
        setTimestamps(newTimestamps);
        storeTimestamps(newTimestamps);
        ...
        setFeedbackText('');
    }, [feedbackText, isOnCooldown, timestamps]);
                ...
                <AnimatedSection delay={0.05}>
                    <div className="feedback-form">
                        <div className="feedback-form__input-wrapper">
                            ...
                            />
                            {!isOnCooldown && messagesLeft < RATE_LIMIT_MAX && (
                                <span className="feedback-form__counter">
                                    {t('kanban.messagesLeft', { count: messagesLeft })}
                                </span>
                            )}
                            {isOnCooldown && (
                                <span className="feedback-form__cooldown">
                                    {t('kanban.cooldownMessage', { minutes: cooldownMinutes, seconds: cooldownSeconds })}
                                </span>
                            )}
                        </div>
                        <button className="btn btn-primary" onClick={handleFeedback} disabled={isOnCooldown}>
                    ...
                    <div className="kanban-board">
                        {columns.map((col) => (
                            <div key={col.key} className="kanban-column">
                                ...
                                <div className={`kanban-column__body ${col.scrollable ? 'kanban-column__body--scrollable' : ''}`}>
                                    {col.items.map((task) => (
