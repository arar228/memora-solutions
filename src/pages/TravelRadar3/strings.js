...
export const STR = {
    ru: {
        ...
        toursTitle: 'Горящие туры',
        toursDesc: 'Выбери город вылета — покажем горящие туры с максимальной экономией. Дата вторична: под хорошую цену проще подстроиться, а отфильтровать по месяцу можно ниже.',
        stitchBtn: 'Готов к стыковкам',
        fMonth: 'Месяц',
        toursEmpty: 'Из этого города горящих туров сейчас нет. Нажми «Готов к стыковкам» — покажем туры из соседних городов с перелётом до них.',
        toursEmptyStitched: 'Пока нет туров даже со стыковками — лента обновляется каждые пару часов, загляни позже.',
        legLabel: 'перелёт',
        legUnknown: 'цену смотри на Aviasales',
        legTicket: 'билет',
        nights: 'ноч.',
        tourBook: 'К туру',
        toursFoot: 'Стыковка — это отдельный перелёт до города вылета тура: показываем её, только если билет не съедает экономию тура. Билеты на стыковку — на Aviasales (ссылка партнёрская), тур бронируется у источника.',
    ...
    en: {
        ...
        toursTitle: 'Hot tours',
        toursDesc: 'Pick your departure city — we show hot tours sorted by savings. The date is secondary: it is easier to adjust to a great price, and you can filter by month below.',
        stitchBtn: 'Open to connections',
        fMonth: 'Month',
        toursEmpty: 'No hot tours from this city right now. Hit "Open to connections" — we will show tours from nearby cities with a flight to them.',
        toursEmptyStitched: 'No tours even with connections yet — the feed refreshes every couple of hours, check back later.',
        legLabel: 'flight',
        legUnknown: 'see the price on Aviasales',
        legTicket: 'ticket',
        nights: 'nights',
        tourBook: 'To the tour',
        toursFoot: 'A connection is a separate flight to the tour departure city — shown only when the ticket does not eat the tour savings. Connection tickets are on Aviasales (affiliate link); the tour is booked at the source.',
