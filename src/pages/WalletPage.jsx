...
export default function WalletPage() {
    ...
    return (
        <div className="product-page product-page--wallet">
            <div className="container">
                <AnimatedSection>
                    <div className="product-page__header">
                        ...
                        <a
                            href="https://t.me/MemoraWallet_bot"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="btn btn-primary product-page__cta"
                        >
                            <Send size={18} /> {t('openBot')} {t('products.wallet.bot')}
                        </a>
                    </div>
                </AnimatedSection>
                ...
                <div className="product-bento">
                    ...
                    <AnimatedSection delay={0.05} className="bento-card bento-card--about">
                        <h2>
                            <Wallet size={24} /> {t('products.wallet.about.title')}
                        </h2>
                        <p className="about-desc">{t('products.wallet.about.desc')}</p>
                    </AnimatedSection>

                    {/* Features Bento Card */}
                    <AnimatedSection delay={0.15} className="bento-card bento-card--features">
                        <h2>
                            <CheckCircle2 size={24} /> {t('products.wallet.features.title')}
                        </h2>
                        <ul className="product-features">
                            {features.map((f, i) => (
                                ...
                                >
                                    ...
                                    <span>{t(`products.wallet.features.${f}`)}</span>
                                </motion.li>
                            ))}
                        </ul>
                    </AnimatedSection>
                    ...
                    <AnimatedSection delay={0.2} className="bento-card bento-card--howto">
                        <h2>{t('products.wallet.howTo.title')}</h2>
                        <ol className="product-steps">
                            {steps.map((s, i) => (
                                <li key={s}>
                                    <div className="step-number">{i + 1}</div>
                                    <div className="step-content">
                                        {t(`products.wallet.howTo.${s}`)}
                                    </div>
                                </li>
                            ))}
                        </ol>
